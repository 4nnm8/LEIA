...
/** Récupère les paramètres passés dans l'URL **/
  ...
  function getParam(par){
	var reg = new RegExp('.*[?&]'+par+'=([0-9]{1,2}).*','gi'),
...
////////// DÉBUT LÉIA //////////////////////////////////////////////////////////////////////////
  ...
  if (c == 1) {
    ...
    loadScript('responsivevoice.js',function(){
	  speak('La synthèse vocale est prête !')
    });
  } else { 
    voice = false;
  }
  ...
  function init(){
    ...
    addEvent(document,'keydown',function(e){
	  ...
	  if (((e.altKey && e.ctrlKey) || e.altLeft)&&(k==59 || k==190 || k==186 || k==110)){  // 225 linux
	    middot();
		e.preventDefault();
	  }
      if (e.altKey && e.shiftKey){
	    switch (k) {
	      ...
	      case 107 : rt = Math.round((rt+0.2)*10)/10; speak('Débit augmentée à '+rt);break; //!\ [+] Vérifier la compatibilité cross-browser
	      case 109 : rt = Math.round((rt-0.2)*10)/10; speak('Débit diminuée à '+rt);break;  //!\ [-] Vérifier la compatibilité cross-browser
        }
        e.preventDefault();
      }
    });
...
/** FONCTION D'ORALISATION **/
  ...
  if (voice) {
    document.querySelectorAll('textarea,input[type=text],input[type=password],input[type=date],input[type=number]').forEach(function(elem){
      addEvent(elem,'keypress',function(event){
